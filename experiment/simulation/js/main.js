...
function startSimulation2(arrival_rate, param1, param2, speed) {
  ...
  let valid = check(arrival_rate,param2-param1,q_size,ns);
  if (!valid) return;
  ...
  chartId = setInterval(function () {
    ...
    if (cust && sTime < nextTime) {
        ...
        if (q.isEmpty) {
            ...
            start_times[idx] = q.dequeue();
            U = Math.random();
            end_times[idx] = time + (t_start + U * t_range);
            av_st += (t_start + U * t_range);
        }
        // update service end time
        sTime = end_times[0];
        for (let i = 1; i < ns; i++) {
...
function drawTable() {
    ...
    // document.getElementById("th_qt").innerHTML = ((th_qt>=0 && th_qt!=NaN)? th_qt.toFixed(2): "The value is not applicable for given inputs");
...
function startSimulationBasedOnDistribution() {
  var serviceDistribution = document.getElementById('sd').value;
  console.log(serviceDistribution);

  var arrivalRate = document.getElementById('ar').value;
  var serviceParameter = document.getElementById('par1').value;
  var speed = 2000;
  ...
  if (serviceDistribution === 'Deterministic') {
    ...
    startSimulation2(arrivalRate, serviceParameter, param2, speed);
  }
  else{
